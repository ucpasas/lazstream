...
export function parseLasHeader(view: DataView): LasHeader {
  // LAS 1.4 header reads up to byte 254 (uint64 point count at offset 247).
  // Anything smaller is either a non-LAS file or a truncated download.
  if (view.byteLength < 255) {
    throw new ParseError(
      `File too small to be a valid LAS file (${view.byteLength} bytes). ` +
      `Minimum LAS header is 227 bytes.`
    )
  }
  ...
  if (pointDataRecordFormat > 10) {
    throw new ParseError(
      `Unsupported point data record format ${pointDataRecordFormat}. ` +
      `Valid formats are 0–10.`
    )
  }
  ...
  if (!Number.isFinite(scaleX) || !Number.isFinite(scaleY) || !Number.isFinite(scaleZ)
    ...
    throw new ParseError(
      `Invalid scale factors in LAS header (${scaleX}, ${scaleY}, ${scaleZ}). ` +
      `Scale must be non-zero finite values.`
    )
  }
  ...
  if (!Number.isFinite(offsetX) || !Number.isFinite(offsetY) || !Number.isFinite(offsetZ)) {
    throw new ParseError(
      `Invalid coordinate offsets in LAS header (${offsetX}, ${offsetY}, ${offsetZ}). ` +
...
export async function fetchAndParseLasHeader(
  ...
  // Cap at 1 MB — no legitimate LAS file has a 1 MB header block.
  // An implausibly large value indicates a malformed or malicious file.
  const MAX_HEADER_BYTES = 1024 * 1024
  if (header.pointDataOffset > INITIAL_FETCH) {
    if (header.pointDataOffset > MAX_HEADER_BYTES) {
      throw new ParseError(
        `Header offset to point data (${header.pointDataOffset} bytes) is implausibly large. ` +
