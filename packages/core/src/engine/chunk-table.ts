 ...
 *
 * All chunk-table arithmetic uses uint32 (Uint32Array + `>>> 0` accumulation)
 * to match C++ U32 wrap semantics. For files ≥ 2 GB, cumulative byte offsets
 * exceed 2^31 — signed JavaScript arithmetic would produce wrong offsets there.
 */
...
export async function fetchChunkTable(
  ...
  // 5 million chunks × 50K pts = 250 billion points — already absurd.
  // Guard before any allocation to prevent OOM on malformed files.
  const MAX_CHUNK_COUNT = 5_000_000
  if (chunkCount > MAX_CHUNK_COUNT) {
    throw new ChunkTableError(
      `Chunk count ${chunkCount} exceeds maximum (${MAX_CHUNK_COUNT}). ` +
  ...
  // `(a + b) >>> 0` keeps accumulation in uint32 space — required for files ≥ 2 GB.
